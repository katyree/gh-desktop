...
| 32 | Reset a branch and inspect reflog or unreachable commits; use `app/src/ui/reset`, `app/src/ui/history/unreachable-commits-dialog.tsx`, `app/src/lib/git/reset.ts`, and `reflog.ts`. | 16, 25 | Partial | Native History captures a full selected commit ID, offers Soft/Mixed/Hard reset with branch/HEAD/target confirmation and Core expected-HEAD protection, and shows a bounded HEAD reflog with explicit Mixed recovery. Hard reset with uncommitted changes now shows a second destructive confirmation (Electron warning-before-reset parity) driven by fresh status with fail-safe toward warning and a revalidation after it; soft/mixed need no extra confirmation. A copied published artifact rendered History on a dirty fixture without mutating it. Dialog-click verification, unreachable-commit browsing, and desktop mutation confirmation remain unverified, so the row stays `Partial`. |
