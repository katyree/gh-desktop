...
| 8 | Implement the app menu, action dispatcher, and keyboard shortcuts; use `app/src/ui/app-menu`, `app/src/ui/dispatcher`, `app/src/lib/menu-item.ts`, and `app/src/ui/keyboard-shortcut`. | 3 | Partial | Native File and View menus expose the supported actions for create, open, clone, Settings, Changes, History, repository chooser, Branches, and Worktrees with the matching Ctrl accelerators. The nine entries share `AppCommandMenuItem_Click` and `ExecuteAppCommandAsync`, preserve their existing availability guards, and route failures to the visible `ShowError` path. Named menu items now mirror repository, busy, mutation, navigation, and folder-picker availability from the same state updates used by the controls. Integration entries keep their direct handlers and synchronized availability. The Release `win-x64` publish built with 0 warnings and 0 errors, the full Core suite passes 215/215, and the published XBF, PRI, icon, Codex, Git, and native executable files are present in `native/artifacts/task73-menus-win-x64`. Electron's broader Edit, Repository, Branch, and additional View commands remain unimplemented. Interactive menu opening and command invocation remain unverified in this session. |
