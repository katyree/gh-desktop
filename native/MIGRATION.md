...
| 7 | Clone a GitHub or generic repository; use `app/src/ui/clone-repository` and `app/src/lib/git/clone.ts`. | 4, 46 | Partial | Native Clone shows progress and opens the resulting repository. The `9B98` artifact cloned a synthetic local source into `native/artifacts/native-clone-ui-20260905`, then opened clean `main` at `origin/main` with `In sync`; independent Git inspection confirmed a clean status and HEAD `892b61e81d6291391afa3501c48e861cb0ade0cc`. Clone failures now map through the shared host-naming transport mapper (SAML, expired, HTTPS-auth, SSH, certificate, unreachable) instead of raw Git output, proven by an unreachable-host test that preserves the original failure. Authenticated hosted verification remains out of scope. |
| 8 | Implement the app menu, action dispatcher, and keyboard shortcuts; use `app/src/ui/app-menu`, `app/src/ui/dispatcher`, `app/src/lib/menu-item.ts`, and `app/src/ui/keyboard-shortcut`. | 3 | Partial | A source audit confirms exact parity for the nine Electron shortcuts implemented by native: `Ctrl+N` create, `Ctrl+O` open, `Ctrl+Shift+O` clone, `Ctrl+,` Settings, `Ctrl+1` Changes, `Ctrl+2` History, `Ctrl+T` repository chooser, `Ctrl+B` Branches, and `Ctrl+Alt+W` Worktrees. XAML accelerators and the comma key registered by `InitializeApplicationCommandAccelerators` route through the shared `AppCommandMenuItem_Click` and `ExecuteAppCommandAsync` path with the existing availability guards and visible `ShowError` failure path. Named menu items retain task 73's synchronized repository, busy, mutation, navigation, and folder-picker availability. The only other native keyboard handlers are `Ctrl+F` diff search, Enter/Shift+Enter/Escape within diff search, and Enter in the repository chooser; the audit found no duplicate shortcut binding or source-proven text-editing conflict. The Release `win-x64` publish built with 0 warnings and 0 errors, the full Core suite passes 215/215, and the published XBF, PRI, icon, Codex, Git, and native executable files are present in `native/artifacts/task74-shortcuts-win-x64`. Electron's broader Edit, Repository, Branch, and additional View shortcuts remain unimplemented. Interactive shortcut activation and text-editing conflict behavior remain unverified because native UI key injection is unavailable in this session. |
| 9 | Handle application URLs, external editors, and shell commands; use `app/src/lib/parse-app-url.ts`, `app/src/ui/open-with-external-editor`, `app/src/ui/editor`, and `app/src/ui/shell`. | 3, 8 | Partial | Native startup positional argv parses supported `wingit://openrepo` links, scans recent repositories by sanitized remote URL, opens or clones with the requested URL prefilled, fetches branch or pull request refs, and guards requested file paths before File Explorer launch. A copied Release publish launched against an isolated localhost remote opened branch `feature/url-routing`, routed pull request 42 to the expected head beginning `e99adb`, and kept Refresh, Open repository, and navigation enabled after the route settled. The Clone dialog prefilled the requested remote and branch. Malformed `wingit:` and other non-file URI inputs show a generic visible error, while normal local filesystem paths continue through repository-open handling. Pull-request routing refuses an existing local `pr/N` branch when its tip differs from the fetched head. Fork/upstream tracking remains unimplemented. Native has guarded repository and selected-file editor, shell, and File Explorer actions with discovered-tool persistence and repository-path containment. An Electron audit confirms that built-in editor launch also passes the full repository or file path; native matches that path handling. A source audit covers the built-in Windows shell types listed by Electron: Electron supplies per-shell target arguments and a working directory, while native passes direct argument arrays and the repository as `WorkingDirectory`; native Cygwin uses mintty `--dir <repository> -` instead of a shell-interpolated command. Native does not add Electron's optional Command Prompt Git alias. A copied x64 artifact rejected a `filepath=../outside.txt` URL with the generic visible message `The requested file could not be revealed.` Desktop proof for external editor and terminal launch, protocol registration, second-instance activation, and full Electron parity remain unverified. |
...
| 32 | Reset a branch and inspect reflog or unreachable commits; use `app/src/ui/reset`, `app/src/ui/history/unreachable-commits-dialog.tsx`, `app/src/lib/git/reset.ts`, and `reflog.ts`. | 16, 25 | Partial | Native History captures a full selected commit ID, offers Soft/Mixed/Hard reset with branch/HEAD/target confirmation and Core expected-HEAD protection, and shows a bounded HEAD reflog with explicit Mixed recovery. Recent branches from the HEAD reflog are now listed too (Electron `getRecentBranches` parity): `GetRecentBranchNamesAsync` parses checkout/rename moves with rename-intermediate exclusion, empty for unborn repositories, and bounded limits; the Branches view shows a Recent branches jump box with the current branch excluded and deleted branches filtered, selecting a name jumps to its row. `GitRepositoryResetReflogTests` covers order, limit, rename exclusion, unborn, and limit validation (4/4) and the full Core suite passes 167/167. A copied published artifact rendered the Recent box on a checkout fixture without mutating it. Dropdown-open and jump-click verification, unreachable-commit browsing, and desktop mutation confirmation remain unverified, so the row stays `Partial`. Unreachable-commit browsing is now covered too: the omitted-commits action opens a Commit reachability dialog listing both unreachable commits (their changes are not shown, with ancestry-path explanation and counts) and reachable commits (included in the combined diff), matching Electron's unreachable-commits dialog tabs. A copied published artifact rendered History with the new code without mutating the fixture. Reachability dialog clicks and desktop mutation confirmation remain unverified, so the row stays `Partial`. |
| 33 | Create and delete tags; use `app/src/ui/create-tag`, `app/src/ui/delete-tag`, and `app/src/lib/git/tag.ts`. | 25 | Partial | Native Tags lists local tags, creates an annotated tag at current HEAD or a selected History commit, confirms deletion by name and target, uses the Core object-ID guard, and refreshes the list. On the `9B98` artifact, `native-verification-tag` was created at the selected target and deleted again without changing HEAD. The create dialog now refuses a duplicate name against a fresh tag read before mutating (Electron create-tag parity; Core git failure stays authoritative for races). A capture-index fix (tags view targeted nav item 5/remotes instead of 6/tags; remotes likewise pointed at submodules) was required for valid evidence: the fixed capture renders the annotated `v1.0.0` list row, target/object IDs, and `Loaded 1 local tags` without mutating the fixture. Broader tag-list refresh and dialog-click verification remain, so the row stays `Partial`. |

## 5. Remotes, transport, and GitHub workflows

| # | Work unit and source | Depends on | Native status | Acceptance criteria |
| --- | --- | --- | --- | --- |
| 34 | Manage remotes and fork contribution targets; use `app/src/ui/repository-settings/remote.tsx`, `fork-settings.tsx`, and `app/src/lib/git/remote.ts`. | 11, 46 | Partial | A user can view, add, edit, and remove remotes through the Remotes view with credential-redacted URLs, name/URL validation, guarded ref deletion, and list refresh; the Core suite covers local-transport add, edit, push, fetch, pull, and fast-forward refusal. `UpdateRemoteHeadAsync` now mirrors Electron's best-effort `updateRemoteHEAD` (`remote set-head -a`, tolerated absence, resolved default-branch name) for fetch/pull to wire in later. Fork contribution targets are now managed per repository: the Parent/Self choice (default Parent) persists in native settings with alias-style normalization and cleanup, the Remotes view shows the fork section with its effective-target description whenever an `upstream` remote exists (the local fork signal until GitHub API detection arrives), and the push dialog names the pull-request target before pushing. A copied published artifact rendered the fork section with Parent selected and the upstream target text without mutating the fixture. Interactive radio/push-dialog verification and API-based fork detection remain open, so the row stays `Partial`. |
| 35 | Publish a local repository; use `app/src/ui/publish-repository` and `app/src/lib/git/init.ts` and `remote.ts`. | 6, 34, 46 | Partial | Publishing creates or selects the remote, pushes the initial branch, and shows the resulting remote state. Local publishing is now implemented without the GitHub repository-creation API: `PublishBranchAsync` creates a missing remote (URL required), refuses to retarget an existing remote to a different URL, requires the published branch checked out with commits, and pushes with upstream tracking; transport failures name the host. The Remotes view has a Publish branch action with a name/URL dialog, and the refreshed remote list shows the resulting state. `GitRepositoryRemotesTests` proves remote creation, upstream tracking, mismatch/unborn/missing-URL refusals, and the full Core suite passes 202/202. A copied published artifact rendered the Publish action without mutating the fixture. GitHub repository creation and hosted verification stay open, so the row is `Partial` instead of `Remaining`. |
| 36 | Fetch remotes; use `app/src/ui/toolbar/push-pull-button.tsx` and `app/src/lib/git/fetch.ts`. | 34, 46 |
Partial | Fetch updates remote refs, reports progress and authentication errors, and leaves local worktree files unchanged. Fetch now refreshes the remote default-branch HEAD symref as best-effort post-step work (Electron parity; a successful fetch stands when the refresh is unavailable) and maps transport failures through the SAML, expired-credential, HTTPS-auth, SSH, certificate, and unreachable identifiers so the error always names the remote host, preserving the original failure otherwise. `GitRepositoryRemotesTests` proves symref restore with dirty-worktree preservation and unreachable-host naming with state intact, and the full Core suite passes 192/192. Per-file LFS transfer progress display stays with later transfer UI work, so the row is `Partial` instead of `Remaining`. |
| 37 | Pull changes and handle pull-required or merge states; use `app/src/lib/git/pull.ts`, `app/src/ui/push-needs-pull`, and `app/src/ui/multi-commit-operation`. | 34, 36, 46 | Partial | Pull fast-forwards or reports the required merge or rebase path, with no silent overwrite of local changes. Divergence now reports that a fast-forward is impossible with the merge-or-rebase path instead of raw Git output, dirty worktrees get a preservation-first refusal, transport failures reuse the shared host-naming mapper, and the remote HEAD symref refreshes on success. `GitRepositoryRemotesTests` proves divergence reporting, dirty preservation, unreachable naming, and symref refresh with state intact, and the full Core suite passes 195/195. Per-file LFS progress display and hosted verification stay open, so the row is `Partial` instead of `Remaining`. |
| 38 | Push changes and handle rejection or force-push guards; use `app/src/lib/git/push.ts`, `app/src/ui/workflow-push-rejected`, `app/src/ui/push-needs-pull`, and `app/src/ui/multi-commit-operation/dialog/warn-force-push-dialog.tsx`. | 34, 36, 46 | Partial | Push reports success or the rejected ref, and force push requires a clear confirmation that names the affected branch. A rejected ref now reports `origin/branch` with pull-first guidance instead of raw Git output, and transport failures reuse the shared host-naming mapper. Force push is implemented with `--force-with-lease` on a freshly captured remote tip (plain `--force` is never used): the push dialog offers a force checkbox, a second destructive confirmation names the affected branch with the lease tip and safe-default button, a missing remote branch refuses force as unnecessary, and a stale lease fails with re-capture guidance instead of overwriting. `GitRepositoryRemotesTests` proves lease replacement, stale-lease refusal with the remote preserved, and remote-tip reads, and the full Core suite passes 199/199. Publish completes with the dialog code. Interactive force-push clicks and hosted verification stay open. |
| 39 | List, open, and inspect pull requests and merge status; use `app/src/ui/branches/pull-request-list.tsx`, `app/src/ui/open-pull-request`, `app/src/ui/pull-request-quick-view.tsx`, and `app/src/lib/stores/pull-request-store.ts`. | 34, 38, 42 | Partial | A selected branch shows its pull requests, changed files, review state, and merge status from the connected account. The Overview detail now names the merge status from GitHub's mergeable report (mergeable/not mergeable with the reported state, or still-computing with refresh guidance); both list and read paths share one parser, and a read round-trip test proves absent mergeable fields surface as the computing state. Live account, network, creation, review submission, checkout, and merge writes remain unverified. |
| 40 | Show, rerun, and notify about pull-request checks; use `app/src/ui/check-runs`, `app/src/ui/notifications`, and `app/src/lib/ci-checks/ci-checks.ts`. | 39 | Partial | Native Core and Remotes source lists legacy statuses and modern check runs, loads job steps, exposes a check link, and provides a guarded re-run action with loading, empty, error, and cancellation states. Check-run details now state the run duration in Electron's short notation (or "Running since" for incomplete runs) from the already-parsed timestamps, with started/completed coverage added to the checks tests. Re-running a check now confirms the target scope first (Actions job vs whole suite) with a safe-default dialog, since reruns consume real CI resources. Failed checks are now surfaced like Electron's failed-checks notification: a failure count plus the first failed modern run selected so its steps load, with shared failure classification (completed failure/action-required, legacy failure/error) covered by a dedicated test. Live GitHub account and network behavior, rerun results, and OS-level notifications remain unverified. |
| 41 | Show repository rules and secret-scanning results; use `app/src/ui/repository-rules` and `app/src/ui/secret-scanning`. | 39 | Partial | A pull request or branch displays applicable rules and secret findings with links to the relevant GitHub detail. The pull request Overview now shows the base branch commit-message rules with a safe rules-page link: rule descriptions (with bypass notes) load alongside files and reviews under the existing generation guards, with loading, empty, partial, error, and cancellation states, and the link builder escapes every segment while pinning the host. Secret metadata is now parsed too: push-protection denials yield typed findings (description, commit/path/line locations, bypass reference, approval need) with secret values never read, and push failures surface a findings summary naming types and first locations. `GitHubSecretScanningTests` covers multi-secret parsing, non-denials, and the summary, and the full Core suite passes 207/207. Per-secret bypass dialogs and hosted verification stay open. |

## 6. Authentication and local tool access

| # | Work unit and source | Depends on | Native status | Acceptance criteria |
| --- | --- | --- | --- | --- |
| 42 | Sign in to GitHub, switch accounts, sign out, and complete callbacks; use `app/src/ui/sign-in`, `app/src/ui/preferences/accounts.tsx`, `app/src/lib/auth.ts`, and `app/src/main-process/ipc-main.ts`. | 1, 9 | Partial | Native Settings lists, loads, and removes DPAPI-backed accounts and offers an explicit device-code flow behind the public `WINGIT_GITHUB_CLIENT_ID` configuration boundary, with optional `WINGIT_GITHUB_HOST` for an Enterprise host. The signed-out/unconfigured capture is verified; live device approval, profile verification, and interactive sign-out still need desktop verification. No Electron profile is read or imported. Poll responses reporting `incorrect_device_code` now map to the Expired session error ("Start again") instead of a generic invalid-response error, covered by an offline device-flow test. A current-source settings capture confirms startup with this change. Account switching and sign-out were audited with no change needed: the store upserts by origin/ID with concurrency and encryption coverage, removal confirms and clears selection with list refresh, selection loads carry generation guards, per-action choosers prefer the Settings-selected account with store reload at use time and clear errors for removed accounts, and sign-out matches Electron's local-only removal. |
| 43 | Handle HTTPS credentials and Git Credential Manager; use `app/src/ui/generic-git-auth`, `app/src/lib/git/authentication.ts`, and `credential.ts`. | 34, 46 | Partial | A credential request identifies the remote host, uses the configured helper or prompt, and never displays a credential value. The credential foundation is now implemented: every owned Git process runs with `GIT_TERMINAL_PROMPT=0` (never prompts, even as a fallback), the fill/approve/reject helper protocol clears inherited helpers first (protocol parsing with `[]` expansion, forbidden-character rejection), and authentication failures map to host-naming messages (auth/prompt/repo-missing signatures; existing output redaction keeps credential values out of errors). `GitRepositoryCredentialsTests` proves a real store round-trip plus the non-interactive miss message, and the full Core suite passes 184/184. The username/password prompt UI stays with the fetch/pull flows that trigger it, so the row is `Partial` instead of `Remaining`. |
| 44 | Handle SSH hosts, keys, passphrases, and passwords; use `app/src/ui/ssh` and `app/src/lib/ssh`. | 34, 46 |
Partial | SSH setup and failures identify the host and next action while keeping private key and password values out of the UI and logs. Trust parsing and failure identification are now implemented with no prior native code: `ParseSshHostPrompt` extracts host, address, key type, and display-safe fingerprint from OpenSSH unknown-host prompts (Electron parser parity), and `IdentifySshFailure` maps host-key, public-key, and unreachable signatures to host-naming next actions without echoing process output. `GitRepositorySshTests` covers documented prompt shapes and every signature, and the full Core suite passes 187/187. The askpass trampoline, passphrase/password stores, and trust dialogs ride with the fetch/pull flows that trigger them, so the row is `Partial` instead of `Remaining`. |
| 45 | Handle SAML reauthentication, expired tokens, and untrusted certificates; use `app/src/ui/saml-reauth-required`, `invalidated-token`, `untrusted-certificate`, and `app/src/lib/git/authentication.ts`. | 42, 43, 44 | Partial | Each error offers the matching recovery path and blocks transport until the required user action completes. SAML enforcement and expired-credential detection are now implemented: `TryParseSamlEnforcement` extracts the organization from the remote SSO message (Electron regex parity, single-line safe), and `IdentifyExpiredCredential` maps bad/invalid/expired-token and 401 signatures to a sign-in-again recovery message naming the host, with generic auth failures explicitly left to the HTTPS helper. `IdentifyCertificateFailure` maps self-signed/issuer/expired/verify-failed signatures to a host-naming recovery that never disables verification, with trust decisions left to the user. `GitRepositorySamlReauthTests` covers all three identifiers, and the full Core suite
passes 190/190. Live SSO/revocation/TLS flows need hosted services, and the re-authorization dialogs ride with the transport flows
that trigger them, so the row stays `Partial`. |
...
| 48 | Create, apply, pop, drop, and switch stashes; use `app/src/ui/stash-changes`, `app/src/ui/stashing`, and `app/src/lib/git/stash.ts`. | 11, 46 | Partial | Stash actions identify the affected changes, handle conflicts, and refresh the worktree and stash list. Create stash is now enabled only with uncommitted changes (Electron only stashes when changes exist; the Core no-changes refusal stays authoritative for races). Paired captures prove both states: disabled with `CHANGES 0`, enabled with `CHANGES 1`, fixtures unmutated. Pop is now implemented too: `PopStashAsync` resolves the entry by immutable SHA, applies it, and drops the guarded ref on success; conflicts retain the entry and report conflict paths (deliberately safer than Electron's drop-on-conflict pop). The Stashes view has a Pop selected action with a confirmation naming the entry and both outcomes. `GitRepositoryWorktreeStashTests` covers pop success, conflict retention, and missing-entry refusal, and the full Core suite passes 170/170. A copied published artifact rendered the stash entry with the Pop action without mutating the fixture. Interactive pop-click verification remains open, so the row stays `Partial`. |
| 49 | Add, delete, rename, and select worktrees; use `app/src/ui/worktrees`, `app/src/ui/toolbar/worktree-dropdown.tsx`, and `app/src/lib/git/worktree.ts`. | 11, 47 | Partial | Worktree operations update the list and current path while refusing unsafe deletion of a checked-out worktree. Creating a worktree directly on a branch that is already checked out in another worktree is now refused before any state change, naming the branch and worktree path (creating a new `-b` branch from it stays allowed). Renaming is now implemented too: `MoveWorktreeAsync` moves a registered secondary worktree to a clean destination, refusing the main worktree, the active worktree (which fails confusingly in Git), unregistered paths, and existing destinations; the Worktrees view has a Rename selected action with confirmation and the same current-worktree gating as removal. `GitRepositoryWorktreeStashTests` covers move success plus all four refusals and the full Core suite passes 172/172. A copied published artifact rendered both worktrees with the Rename action without mutating the fixture. Interactive rename-click verification remains open, so the row stays `Partial`. |
| 50 | Show submodule state, update submodules, and open submodule diffs; use `app/src/lib/git/submodule.ts` and `app/src/ui/diff/submodule-diff.tsx`. | 11, 13 | Partial | A repository with submodules identifies each path and revision, and update errors identify the submodule that failed. Updates now run recursively like Electron's `updateSubmodulesAfterOperation`, bringing nested submodules to their recorded commits; `GitRepositorySubmodulesTests` proves a nested advance lands exactly (child and nested HEADs, file bytes, staged-only remainder) and the full Core suite passes 173/173. Recursive verification is covered; network, uninitialized-update transport, and broader failure cases remain unverified, so the row stays `Partial`. |
| 51 | Initialize and operate Git LFS with progress; use `app/src/ui/lfs`, `app/src/lib/git/lfs.ts`, and `app/src/lib/progress/lfs.ts`. | 34, 46 | Partial | LFS setup reports availability and progress, and LFS transport errors identify the affected operation. Detection and repository initialization are now implemented with no prior native code: `GetLfsVersionAsync` reports the installed filter version or null, `IsUsingLfsAsync` reads tracked patterns without installing hooks (false when LFS is unavailable or unreadable), `IsTrackedByLfsAsync` checks path coverage through attributes, and `InstallLfsHooksAsync` installs repository hooks with an optional force flag; global filter installation stays out of scope. `GitRepositoryLfsTests` covers version, usage/path tracking (including subdirectory basenames), and hook installation, and the full Core suite passes 176/176. Transfers with progress and the initialize prompt stay with the next slice, so the row is `Partial` instead of `Remaining`. Transfers are now implemented: `FetchLfsObjectsAsync` runs `git lfs fetch` (optional remote and `--all`) with a `GIT_LFS_PROGRESS` file whose documented lines parse into per-file updates with running totals (Electron `GitLFSProgressParser` parity), and LFS failures throw errors naming the fetch operation (smudge/download, batch/auth, and missing-binary signatures identified; other output falls back to the first Git error line). Verified with real local transfers: a skip-smudge clone holds only the pointer, the service fetch materializes the 51200-byte object, and path tracking confirms coverage; the parser and failure-identification units cover documented shapes. Live progress lines were not observable from fast local transfers (empty file, still cleaned up), so hosted-transfer progress display rides with the fetch/pull/push tasks; authenticated hosted verification is out of scope. |
...
| # | Work unit and source | Depends on | Native status | Acceptance criteria |
| --- | --- | --- | --- | --- |
| 58 | Provide Codex login, logout, and account state; use `app/src/ui/preferences/codex.tsx`, `app/src/lib/stores/codex-account-store.ts`, and `app/src/main-process/codex-authorization.ts`. | 3 | Partial | The native account screen shows signed-out and signed-in states, offers browser and device-code login, correlates
completion, and keeps credentials outside the Electron profile. Interactive authentication still needs desktop
verification. A login-lane audit found no gap: authorization URLs are host-allow-listed before UI or browser launch,
login completion correlates by opaque ID with late-callback guards, sign-out confirms and clears presentation, and the
device/browser/cancel/logout client paths validate inputs; the Codex suites cover protocol mapping without
credential fields. |
| 59 | Show Codex usage and rate limits; use `app/src/ui/preferences/codex.tsx` and `app/src/main-process/codex-rate-limits.ts`. | 58 | Partial | Native usage bars and reset text cover available,
near-limit, exhausted, and unavailable states without blocking Git workflows. The status derivation now has boundary
coverage for every state (available below threshold, saturation and both explicit exhaustion flags, both-null
unavailable with earliest-reset selection), matching Electron's thresholds. Live account usage still needs a
signed-in verification. |
| 60 | Provide one shared Codex model and reasoning selector; use `app/src/lib/codex-model-selection.ts` and `app/src/ui/preferences/codex.tsx`. | 58 | Partial | Native settings persist one model and effort choice, validate it against a returned catalog, and preserve it when the catalog is unavailable. The commit-message and selected-changes review callers now consume the single shared selection via `TryGetSharedCodexModelSelection` (validated against the catalog, reasoning effort normalized to the model's supported list or its default). |
| 61 | Generate an editable commit message; use `app/src/lib/codex-commit-message-generator.ts`, `app/src/ui/generate-commit-message`, and `app/src/ui/changes/commit-message.tsx`. | 12, 19, 58, 60 | Partial | The native Changes panel sends the exact raw staged patch; amend uses the HEAD-parent-to-index replacement diff. It shows progress/cancellation and safe failure states, guards the repository, full index fingerprint, HEAD, selected model/effort via the shared selector, and draft text, and populates a reviewable title and description without committing. The request now includes best-effort branch commit-message rule descriptions (GitHub API, host-matched account) as constraints. Live Codex generation and interactive acceptance still need verification. |
| 62 | Review merge, rebase, and cherry-pick conflicts with per-file Codex suggestions; use `app/src/lib/codex-conflict-suggestion-generator.ts` and `app/src/ui/multi-commit-operation/dialog`. | 27, 28, 29, 58, 60 | Partial | Native conflict editor exposes a review-only Codex suggestion action gated on sign-in and the shared model selector; results show the affected file and require an explicit per-file checkbox before the suggestion is copied into the editor draft, so no repository write happens from the suggestion alone. Live Codex generation and desktop interaction remain unverified; batch multi-file orchestrator UI beyond the single selected conflict is not wired. |
| 63 | Review selected changes with file and line findings; use `app/src/lib/codex-selected-changes-review-generator.ts` and `app/src/ui/changes/selected-changes-review-dialog.tsx`. | 12, 13, 58, 60 | Partial | Native source captures the selected files and line findings, keeps the review receipt separate from commit-message consent, guards stale selection and repository state, consumes the shared model/reasoning selector, and exposes review-only results without a Git mutation. Focused compiled consent checks cover separation, path normalization, JSON round-trip, expiry, and invalid receipts. Live Codex generation, authentication, and desktop review interaction remain unverified. |
...
Task 84 starts from task 83's `6ac2cbfdc1` signing-gate commit. The native
updater is disabled until `WINGIT_NATIVE_UPDATE_FEED_URL`,
`WINGIT_NATIVE_UPDATE_CHANNEL` (`production`, `beta`, or `test`), and
`WINGIT_NATIVE_UPDATE_SIGNER_SUBJECT` are configured. The signer subject must
match the subject used by the task 83 release gate. The feed must use HTTPS and
return either 204/404 for no update or JSON with `version`, `channel`,
`assetUrl`, and `sha256`. The asset is a ZIP on the same HTTPS host. Beta and
production check at startup and every four hours; a manual check includes
`skipGuidCheck=1`. A stable GUID is stored in the native profile's `.update-id`.
The settings view shows checking, byte download progress, errors, and the
downloaded result. It offers no install action.

Before retaining a download, the client checks the channel and version, the
archive SHA-256, safe ZIP paths, required WinUI files, task 83's
`SigningStatus.json` with `releaseGate=Passed`, its executable hash and signer,
and the executable's real Authenticode status. Failed checks remove the partial
archive. Five focused tests cover manual and background checks, download
progress, no update, hash mismatch, unsigned executable, and a blocked signing
gate. The full Core suite passed 231/231. A Release `win-x64` build and publish
passed with 0 warnings and 0 errors. A copied published app rendered the
disabled-channel Settings state in `native/artifacts/task84-settings-verified.png`;
the copied unsigned `WinGit.Native.exe` returned exit code 1 from the shipped
signature verifier. The task 83 audit gate marked the new published executable
`NotSigned` and `Blocked`, with a matching SHA-256. No hosted feed, real signed
...
If preparation or helper validation fails, it does not replace the current
app. After a helper failure, the old app is relaunched when available and the
result is shown on startup. Nine focused updater tests passed, including
missing, incomplete, and changed downloads and synthetic staging without a
current-app write. A synthetic unsigned helper probe exited 1, preserved the
old executable, and recorded a failure result. The Release win-x64 build and
publish passed with 0 warnings and 0 errors. Its 917-file output contains the
install helper and required app/runtime files. A copied published app showed
...
| 66 | Provide an update channel and update progress; use `app/src/main-process/squirrel-updater.ts`, `app/src/ui/installing-update`, `app/src/ui/lib/update-store.ts`, and `app/src/lib/get-updater-guid.ts`. | 64, 65 | Partial | Task 84 implements discovery, validation, download, and visible progress on a configured signed channel. Task 85 adds explicit install confirmation, a revalidated package handoff, progress while staging, post-exit folder replacement, previous-folder backup, restart, and visible failure result. Missing, changed, incomplete, blocked, or unsigned updates cannot enter the install handoff. Synthetic staging and unsigned rejection passed locally; signed end-to-end replacement, restart, rollback, and a real installer remain unverified. |
