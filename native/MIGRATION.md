...
| 40 | Show, rerun, and notify about pull-request checks; use `app/src/ui/check-runs`, `app/src/ui/notifications`, and `app/src/lib/ci-checks/ci-checks.ts`. | 39 | Partial | Native Core and Remotes source lists legacy statuses and modern check runs, loads job steps, exposes a check link, and provides a guarded re-run action with loading, empty, error, and cancellation states. Check-run details now state the run duration in Electron's short notation (or "Running since" for incomplete runs) from the already-parsed timestamps, with started/completed coverage added to the checks tests. Re-running a check now confirms the target scope first (Actions job vs whole suite) with a safe-default dialog, since reruns consume real CI resources. Live GitHub account and network behavior, rerun results, and failed-check notifications remain unverified. |
