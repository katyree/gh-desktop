...
| 38 | Push changes and handle rejection or force-push guards; use `app/src/lib/git/push.ts`, `app/src/ui/workflow-push-rejected`, `app/src/ui/push-needs-pull`, and `app/src/ui/multi-commit-operation/dialog/warn-force-push-dialog.tsx`. | 34, 36, 46 | Partial | Push reports success or the rejected ref, and force push requires a clear confirmation that names the affected branch. A rejected ref now reports `origin/branch` with pull-first guidance instead of raw Git output, and transport failures reuse the shared host-naming mapper. `GitRepositoryRemotesTests` proves rejection naming with both tips preserved plus unreachable-host naming, and the full Core suite passes 197/197. Force push stays with the next slice, so the row is `Partial` instead of `Remaining`. |
