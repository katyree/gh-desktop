...
| 9 | Handle application URLs, external editors, and shell commands; use `app/src/lib/parse-app-url.ts`, `app/src/ui/open-with-external-editor`, `app/src/ui/editor`, and `app/src/ui/shell`. | 3, 8 | Partial | Supported links open the intended repository or commit, an editor opens the requested file, and a shell opens at the repository root. Native has guarded repository and selected-file editor, shell, and File Explorer actions with discovered-tool persistence and repository-path containment. An Electron audit confirms that built-in editor launch also passes the full repository or file path; native matches that path handling. A source audit covers the built-in Windows shell types listed by Electron: Electron supplies per-shell target arguments and a working directory, while native passes direct argument arrays and the repository as `WorkingDirectory`; native Cygwin uses mintty `--dir <repository> -` instead of a shell-interpolated command. Native does not add Electron's optional Command Prompt Git alias. Electron custom integrations still allow a user path and `%TARGET_PATH%` arguments, which native does not expose. Application URL parsing, desktop editor and terminal launch proof, and full Electron parity remain unverified. |
