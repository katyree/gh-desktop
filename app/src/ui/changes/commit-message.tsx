...
} from '../../lib/stores/codex-account-store'
...
const openAILogo: OcticonSymbolVariant = {
  w: 24,
  h: 24,
  p: [
...
> {
  ...
  private async updateRepoRulesCommitMessageFailures(
    ...
    if (
      ...
      const context: ICommitContext = {
        ...
        messageGeneratedByAI: this.state.commitMessage.generatedByAI ?? false,
      }
  ...
  private onSummaryChanged = (summary: string) => {
    this.setState({
      commitMessage: {
        ...
        // that the commit message was not generated by AI anymore.
        generatedByAI: false,
        timestamp: Date.now(),
      },
    })
  ...
  private getGenerateCommitMessageMenuItem(): IMenuItem | null {
    ...
    return {
      ...
      enabled:
        ...
        !isGeneratingCommitMessage &&
        !noChangesAvailable &&
        this.codexGenerationDisabledReason === null,
    }
  }
  ...
  private renderCopilotButton() {
    ...
    let ariaLabel =
      this.codexGenerationDisabledReason ?? 'Generate commit message with Codex'
  ...
  /** Whether the Codex action should be available in the commit form. */
  private get isCopilotButtonEnabled() {
    return this.props.onGenerateCommitMessage !== undefined
  }

  private get codexGenerationDisabledReason(): string | null {
    if (this.props.codexAccount === undefined) {
      return 'Sign in to ChatGPT in Options to generate a commit message.'
    }
    ...
    switch (availability) {
      ...
      default:
        return assertNever(
          availability,
          'Unknown Codex commit-message availability'
        )
    }
  }
