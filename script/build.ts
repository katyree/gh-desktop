...
// A standalone `tsc` validation emits application modules under out/app. They
// are not runtime inputs—the production bundles are the top-level Webpack
// outputs—and deleted modules can otherwise survive there and be packaged.
rmSync(path.resolve(outRoot, 'app'), { recursive: true, force: true })
...
function packageApp() {
  ...
  return packager({
    ...
    protocols: [
      {
        name: getBundleID(),
        schemes: [...getProtocolSchemes(process.env.NODE_ENV)],
      },
    ],
...
function copyDependencies() {
  ...
  console.log('  Copying Codex runtime…')
  copyCodexDependency()
...
function copyCodexDependency() {
  // Older WinGit builds copied the GitHub Copilot runtime here. Remove that
  // exact legacy payload so an incremental build cannot package stale files.
  rmSync(path.resolve(outRoot, 'copilot'), { recursive: true, force: true })

  const codexPackageDirectory = path.resolve(
    projectRoot,
    `app/node_modules/@openai/codex-${
      process.platform
    }-${getDistArchitecture()}`
  )
  const codexDestination = path.resolve(outRoot, 'codex')
  removeAndCopy(codexPackageDirectory, codexDestination)
}
